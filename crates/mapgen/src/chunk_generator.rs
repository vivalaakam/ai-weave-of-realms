...
use engine::map::chunk::{Chunk, ChunkCoord, CHUNK_TILE_COUNT};
use engine::map::tile::{Tile, Tiles};
use engine::rng::derive_seed;
...
impl ChunkGenerator {
    ...
    ///
    /// # Errors
    /// Returns [`MapgenError::ScriptLoad`] if the file cannot be read or the script
    /// fails to compile / does not return a function.
    pub fn from_script(path: &Path) -> Result<Self, MapgenError> {
        let source = std::fs::read_to_string(path)?;
        let lua = Lua::new();

        let func: Function =
            lua.load(&source).set_name(path.to_string_lossy().as_ref()).eval().map_err(
                |source| MapgenError::ScriptLoad {
    ...
    ///
    /// # Errors
    /// Returns [`MapgenError::LuaExecution`] if the Lua function fails, or
    /// [`MapgenError::InvalidChunkData`] if the returned table is malformed.
    #[instrument(skip(self, map_seed), fields(cx = coord.x, cy = coord.y))]
    pub fn generate(&self, coord: ChunkCoord, map_seed: &[u8; 32]) -> Result<Chunk, MapgenError> {
    ...
    pub fn generate_with_base(
        ...
        let rng = self.lua.create_userdata(LuaRng::new(chunk_seed)).map_err(|source| {
            MapgenError::LuaExecution { function: "create_userdata(LuaRng)".into(), source }
        })?;
            ...
            Some(chunk) => {
                ...
                for (i, tile) in chunk.tiles().iter().enumerate() {
                    table.raw_set(i + 1, tile.kind.as_str()).map_err(|source| {
                        MapgenError::LuaExecution { function: "base_table.set".into(), source }
                    })?;
                }
                Value::Table(table)
            }
        ...
        let tiles_table: Table =
            self.func.call((rng, coord.x, coord.y, base_value)).map_err(|source| {
                MapgenError::LuaExecution { function: "generate_chunk".into(), source }
            })?;
...
///
/// # Errors
/// Returns [`MapgenError::InvalidChunkData`] if the table length is wrong or
/// contains an unrecognised tile kind string.
fn parse_tiles_table(table: Table, coord: ChunkCoord) -> Result<Chunk, MapgenError> {
    let mut tiles = Vec::with_capacity(CHUNK_TILE_COUNT);

    for i in 1..=(CHUNK_TILE_COUNT as i64) {
        let kind_str: String = table
            .get(i)
            .map_err(|_| MapgenError::InvalidChunkData(format!("missing tile at Lua index {i}")))?;

        let kind = Tiles::from_str(&kind_str).map_err(|_| {
            MapgenError::InvalidChunkData(format!("unknown tile kind '{kind_str}' at index {i}"))
        })?;

        tiles.push(Tile::new(kind));
    }

    Chunk::from_vec(coord, tiles).map_err(MapgenError::from)
}
...
mod tests {
    ...
    fn invalid_tile_kind_returns_error() {
        ...
        assert!(matches!(result, Err(MapgenError::InvalidChunkData(_))));
    }
