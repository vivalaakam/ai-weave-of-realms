...
impl MapValidator {
    ...
    ///
    /// # Errors
    /// Returns [`MapgenError::LuaExecution`] if the Lua function raises a runtime error
    /// (distinct from a soft validation failure).
    #[instrument(skip(self, map))]
    pub fn validate(&self, map: &GameMap) -> Result<ValidationResult, MapgenError> {
        let map_table = game_map_to_lua_table(&self.lua, map).map_err(|source| {
            MapgenError::LuaExecution { function: "game_map_to_lua_table".into(), source }
        })?;

        let multi: MultiValue = self
            .func
            .call(map_table)
            .map_err(|source| MapgenError::LuaExecution { function: "validate".into(), source })?;
...
///
/// # Errors
/// Returns [`MapgenError::LuaExecution`] if the first return value is not a boolean.
fn parse_validation_result(mut multi: MultiValue) -> Result<ValidationResult, MapgenError> {
    // MultiValue is ordered: first value = valid, second value = reason
    let valid = match multi.pop_front() {
        Some(mlua::Value::Boolean(b)) => b,
        other => {
            return Err(MapgenError::LuaExecution {
