...
impl EnemySpawner {
    ...
    ///
    /// # Errors
    /// Returns [`MapgenError::LuaExecution`] if the Lua function raises an error.
    #[instrument(skip(self, map))]
    pub fn spawn(&self, map: &GameMap) -> Result<Vec<EnemySpawn>, MapgenError> {
        ...
        let result: Value = self.func.call(map_table).map_err(|source| {
            MapgenError::LuaExecution { function: "spawn_enemies".into(), source }
        })?;
...
fn parse_spawn_results(value: Value) -> Result<Vec<EnemySpawn>, MapgenError> {
...
fn parse_enemy_entry(table: Table, index: u32) -> Result<Option<EnemySpawn>, MapgenError> {
