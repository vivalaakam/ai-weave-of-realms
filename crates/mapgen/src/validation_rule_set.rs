...
impl ValidationRuleSet {
    ...
    ///
    /// # Errors
    /// Returns [`MapgenError::ScriptLoad`] if any `.lua` file cannot be read or compiled.
    /// Returns [`MapgenError::Engine`] with [`InvalidState`](rpg_engine::error::GameError::InvalidChunksSize)
    /// if `dir` cannot be read.
    pub fn from_dir(dir: &Path) -> Result<Self, MapgenError> {
        let mut entries: Vec<_> = std::fs::read_dir(dir)
            .map_err(|e| {
                MapgenError::Engine(engine::error::EngineError::ValidationRuleDir {
    ...
    ///
    /// # Errors
    /// Returns [`MapgenError::LuaExecution`] if a rule raises a runtime error (as opposed
    /// to returning `false` — a soft validation failure).
    #[instrument(skip(self, map))]
    pub fn validate_all(&self, map: &GameMap) -> Result<Vec<RuleResult>, MapgenError> {
