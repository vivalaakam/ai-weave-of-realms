    ...
    fn validate(&self, map: &GameMap) -> Result<(), Error> {
        match self {
            ...
            Validator::Single(v) => {
                ...
                if !result.is_valid() {
                    return Err(Error::ValidationFailed(result.reason.unwrap_or_default()));
                }
                Ok(())
            }
        }
...
impl MapAssembler {
    ...
    pub fn new(config: MapConfig) -> Result<Self, Error> {
        if config.generators.is_empty() {
            return Err(Error::Engine(rpg_engine::error::Error::PipelineEmpty));
        }
    ...
    pub fn generate(&self) -> Result<GameMap, Error> {
        ...
        let mut map = grid.into_game_map().map_err(Error::Engine)?;
...
mod tests {
    ...
    fn pipeline_applies_generators_in_order() {
        ...
        assert_eq!(
            ...
            rpg_engine::map::tile::Tiles::Water
        );
    }
