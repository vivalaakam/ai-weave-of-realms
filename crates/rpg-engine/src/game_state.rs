...
impl GameState {
    ...
    pub fn get_active_team(&self) -> Result<&Team, Error> {
        let active_team = self.get_active_team_id()?;
        self.teams.get(active_team).ok_or(Error::ActiveTeamNotFound(*active_team))
    }

    /// Returns the currently active team id.
    pub fn get_active_team_id(&self) -> Result<&TeamId, Error> {
        self.teams_order.front().ok_or(Error::NoActiveTeam)
    }
    ...
    pub fn on_turn(&mut self) -> Result<TurnEvent, Error> {
        let active_team_id = *self.get_active_team_id()?;
        let Some(team) = self.teams.get_mut(&active_team_id) else {
            return Err(Error::ActiveTeamNotFound(active_team_id));
        };
