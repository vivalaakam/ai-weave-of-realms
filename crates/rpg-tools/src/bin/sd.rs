...
impl StableDiffusionVersion {
    fn repo(&self) -> &'static str {
        match self {
            ...
            Self::IllustriousV40 => "John6666/illustrious-pixel-art-from-hades-v4-series-v-40-sdxl",
        }
    }
...
fn save_image(
    ...
    for batch in 0..bsize {
        ...
        rpg_tools::save_image::save_image(&image, image_filename)?;
    }
    Ok(())
}
...
fn text_embeddings(
    ...
    let text_embeddings = if use_guide_scale {
        let mut uncond_tokens =
            tokenizer.encode(uncond_prompt, true).map_err(E::msg)?.get_ids().to_vec();
...
fn image_preprocess<T: AsRef<std::path::Path>>(path: T) -> anyhow::Result<Tensor> {
    ...
    let img =
        img.resize_to_fill(width as u32, height as u32, image::imageops::FilterType::CatmullRom);
    let img = img.to_rgb8();
...
fn run(args: Args) -> Result<()> {
    ...
    if let Some(parent) = std::path::Path::new(&final_image).parent() {
    ...
    let guidance_scale = match guidance_scale {
        ...
        None => match sd_version {
            ...
            | StableDiffusionVersion::XlInpaint
            | StableDiffusionVersion::Xl
            | StableDiffusionVersion::IllustriousV40 => 7.5,
    ...
    let n_steps = match n_steps {
        ...
        None => match sd_version {
            ...
            | StableDiffusionVersion::XlInpaint
            | StableDiffusionVersion::Xl
            | StableDiffusionVersion::IllustriousV40 => 30,
    ...
    let which = match sd_version {
        ...
        | StableDiffusionVersion::XlInpaint
        | StableDiffusionVersion::Turbo
        | StableDiffusionVersion::IllustriousV40 => vec![true, false],
        _ => vec![true],
    };
    ...
    let t_start =
        if img2img.is_some() { n_steps - (n_steps as f64 * img2img_strength) as usize } else { 0 };
    ...
    let vae_scale = match sd_version {
        ...
        | StableDiffusionVersion::XlInpaint
        | StableDiffusionVersion::Xl
        | StableDiffusionVersion::IllustriousV40 => 0.18215,
