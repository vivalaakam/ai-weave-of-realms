use candle_core::{DType, Device, IndexOp, MetalDevice, Tensor};
use candle_transformers::models::mmdit::model::{Config as MMDiTConfig, MMDiT};

use rpg_tools::clip::StableDiffusion3TripleClipWithTokenizer;
use rpg_tools::vae::{build_sd3_vae_autoencoder, sd3_vae_vb_rename};

use anyhow::{Ok, Result};
use candle_core::backend::BackendDevice;
use clap::Parser;
use rpg_tools::sampling;
use rpg_tools::save_image::save_image;
...
struct Args {
    ...
    /// The file name of the final generated image.
...
fn main() -> Result<()> {
    ...
    let x = {
        let mmdit = MMDiT::new(&mmdit_config, use_flash_attn, vb.pp("model.diffusion_model"))?;
    ...
    save_image(&img.i(0)?, &final_image)?;
    Ok(())
}
