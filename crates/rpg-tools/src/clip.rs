...
impl ClipWithTokenizer {
    fn new(
        ...
        let tokenizer = Tokenizer::from_file(
            path_buf
                .to_str()
                .ok_or(E::msg("Failed to serialize huggingface PathBuf of CLIP tokenizer"))?,
        )
        .map_err(E::msg)?;
        Ok(Self { clip, config, tokenizer, max_position_embeddings })
    }

    fn encode_text_to_embedding(&self, prompt: &str, device: &Device) -> Result<(Tensor, Tensor)> {
        ...
        let (text_embeddings, text_embeddings_penultimate) =
            self.clip.forward_until_encoder_layer(&tokens, usize::MAX, -2)?;
...
impl T5WithTokenizer {
    fn new(vb: candle_nn::VarBuilder, max_position_embeddings: usize) -> Result<Self> {
        ...
        let tokenizer_filename =
            api.model("lmz/mt5-tokenizers".to_string()).get("t5-v1_1-xxl.tokenizer.json")?;

        let tokenizer = Tokenizer::from_file(tokenizer_filename).map_err(E::msg)?;
        Ok(Self { t5: model, tokenizer, max_position_embeddings })
    }

    fn encode_text_to_embedding(&mut self, prompt: &str, device: &Device) -> Result<Tensor> {
        let mut tokens = self.tokenizer.encode(prompt, true).map_err(E::msg)?.get_ids().to_vec();
...
impl StableDiffusion3TripleClipWithTokenizer {
    pub fn new_split(
        ...
        Ok(Self { clip_l, clip_g, clip_g_text_projection: text_projection, t5 })
    }
    ...
    pub fn encode_text_to_embedding(
        ...
        let t5_embeddings =
            self.t5.encode_text_to_embedding(prompt, device)?.to_dtype(DType::F16)?;
