...
impl Chunk {
    ...
    ///
    /// # Errors
    /// Returns [`EngineError::InvalidTiles`] if `tiles` does not contain exactly
    /// [`CHUNK_TILE_COUNT`] elements.
    pub fn from_vec(coord: ChunkCoord, tiles: Vec<Tile>) -> Result<Self, EngineError> {
    ...
    ///
    /// # Errors
    /// Returns [`EngineError::OutOfBounds`] if `x >= CHUNK_SIZE` or `y >= CHUNK_SIZE`.
    pub fn get(&self, x: usize, y: usize) -> Result<&Tile, EngineError> {
    ...
    /// # Errors
    /// Returns [`EngineError::OutOfBounds`] if `x >= CHUNK_SIZE` or `y >= CHUNK_SIZE`.
    pub fn get_mut(&mut self, x: usize, y: usize) -> Result<&mut Tile, EngineError> {
        if x >= CHUNK_SIZE || y >= CHUNK_SIZE {
            return Err(EngineError::OutOfBounds(format!(
                "({x}, {y}) in chunk of size {CHUNK_SIZE}"
            )));
