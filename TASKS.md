...
| 13.13 | `bevy-console`: левый стик двигает курсор, Cross открывает экран города, hover города масштабирует тайл до 3x3                                | Codex    | DONE   | Left stick -> `Cursor*`; добавлен `AppState::City`; city tile под курсором рисуется `3x3`                                                                    |
| 13.14 | Исправить Bevy panic `B0001` после добавления city hover scale                                                                                  | Codex    | DONE   | `tile_query` и `cursor_query` разведены через `With/Without`, чтобы Bevy видел disjoint `Transform` access                                                   |
| 13.15 | Исправить city cursor UX: snap в центр города и большой курсор вместо масштабирования стен                                                       | Codex    | DONE   | `MapViewApp` snap'ит city cursor в центр; Bevy масштабирует `CursorOverlay`, а не city tile                                                                   |
| 13.16 | Исправить выход курсора с города и оставить entrance с маленьким курсором                                                                        | Codex    | DONE   | `CityEntrance` не snap'ится/не масштабируется; движение с `City` идёт до ближайшей не-city клетки в выбранном направлении                                    |
| 13.17 | Добавить наведение игрового курсора мышкой в `bevy-console`                                                                                      | Codex    | DONE   | Mouse position -> visible map tile; shared cursor snap сохраняется; неподвижная мышь не перебивает gamepad/keyboard                                         |
